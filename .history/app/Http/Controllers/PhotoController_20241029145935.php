...
class PhotoController extends Controller
{
    ...
    {
        // Ensure the authenticated user can access only their own albums
        $albums = Album::where('userID', Auth::id())->get();
    
        // Check if the photo belongs to the authenticated user for security
        if ($photo->userID !== Auth::id()) {
            return redirect()->route('albums.index')->with('error', 'Unauthorized access to edit photo.');
        }
    
        // Pass the photo and albums to the edit view
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('home', $request->input('albumID'))->with('success', 'Photo uploaded successfully.');
    }
    ...
    public function update(Request $request, Photo $photo)
{
    ...
    // Update the photo's details without the file path
    $photo->update([
        'judulFoto' => $request->input('judulFoto'),
        'deskripsiFoto' => $request->input('deskripsiFoto'),
        'albumID' => $request->input('albumID'),
    ...
    // Redirect to the album's photos with a success message
    return redirect()->route('albums.photos', ['albumID' => $photo->albumID])->with('success', 'Photo updated successfully.');
}
...
public function like(Photo $photo)
{
    ...
    $like = LikePhoto::where('userID', Auth::id())
                     ->where('fotoID', $photo->fotoID) // Change photoID to fotoID
                     ->first();
    ...
    if ($like) {
        ...
        LikePhoto::create([
            'userID' => Auth::id(),
            'fotoID' => $photo->fotoID, // Change photoID to fotoID
            'tanggalLike' => now(), // Assuming you want to store the like date
        ]);
    }
    ...
    public function showComments(Photo $photo)
{
    ...
    $comments = Comment::where('fotoID', $photo->fotoID)->get();
    
    // Pass the photo and comments to the view
    ...
    public function storeComment(Request $request, Photo $photo)
{
    ...
    Comment::create([
        ...
        'userID' => Auth::id(),
        'fotoID' => $photo->fotoID, // Use fotoID as per your naming convention
    ]);
